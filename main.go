...
func findSmartContext(symbol string) (string, error) {
	...
	strategies := []string{
		...
		titleCase(symbol) + ".md",       // title case
	}
...
func findGlobalModulesInProject(projectDir string) []string {
	deployDir := resolveDeployDir()
	var globalModules []string
	...
	err := filepath.WalkDir(projectDir, func(path string, d fs.DirEntry, err error) error {
		...
		for _, req := range extractRequires(string(content)) {
			if seen[req] {
				continue
			}
			seen[req] = true
			modulePath := strings.ReplaceAll(req, ".", string(filepath.Separator))
			projectFile := filepath.Join(projectDir, modulePath+".lua")
			projectInit := filepath.Join(projectDir, modulePath, "init.lua")
			...
			deployFile := filepath.Join(deployDir, modulePath+".lua")
			deployInit := filepath.Join(deployDir, modulePath, "init.lua")
			if _, err := os.Stat(deployFile); err == nil {
				globalModules = append(globalModules, req)
			} else if _, err := os.Stat(deployInit); err == nil {
				globalModules = append(globalModules, req)
			}
		}
		return nil
