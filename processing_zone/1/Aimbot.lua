...
local function checkPathClearance(dest, direction, angle, distance, origin, target)
    ...
    local shouldhitentity = function(entity) return entity:GetIndex() ~= target:GetIndex() or
        entity:GetIndex() ~= entities.GetLocalPlayer():GetIndex() end
    ...
    local traceLine = engine.TraceLine(origin, point, MASK_SOLID_BRUSHONLY, shouldhitentity)

    -- Perform a trace line downwards from the calculated point to ensure it's on solid ground
    local traceDown = engine.TraceLine(point, point + Vector3(0, 0, -100), MASK_SOLID_BRUSHONLY, shouldhitentity)
    ...
    local visibilityCheck = engine.TraceLine(origin, dest, MASK_SOLID_BRUSHONLY, shouldhitentity)
...
local function FindBestShootingPosition(origin, dest, target, BlastRadius)
    ...
    local function checkPath(direction, angle, distance)
        ...
        local traceLineOriginToPoint = engine.TraceLine(origin, point, MASK_SOLID_BRUSHONLY,
    ...
    local initialTrace = engine.TraceLine(origin, dest, MASK_SOLID_BRUSHONLY,
        function(entity) return entity:GetIndex() ~= target:GetIndex() end)
