...
def _run_luacheck(arguments: dict) -> dict:
    ...
    if check_bundle:
        ...
        # Read file content for policy check
        content = file_path.read_text('utf-8', errors='ignore')
        
        # Check Zero-Mutation policy first
        policy_violations = _check_zero_mutation_policy(content, file_path)
        if policy_violations:
            violation_msg = "Zero-Mutation Lbox policy violation(s) detected:\n"
            for v in policy_violations:
                violation_msg += f"  Line {v['line']}: {v['message']}\n"
            return {
                "file": str(file_path),
                "check_type": "syntax",
                "valid": False,
